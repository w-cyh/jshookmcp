/**
 * Integration tests for the apk-packer sub-domain of binary-instrument.
 *
 * Verifies the domain is discoverable by the registry, the manifest
 * registrations bind correctly, and the tools work end-to-end
...
import manifest from '@server/domains/binary-instrument/manifest';
...
const APK_PACKER_TOOL_NAMES = new Set([
  'apk_packer_detect',
...
describe('apk-packer integration', () => {
  it('apk-packer tools are registered under binary-instrument domain', async () => {
    const apkPackerRegs = manifest.registrations.filter((r) =>
      APK_PACKER_TOOL_NAMES.has(r.tool.name),
    );
    const toolNames = apkPackerRegs.map((r) => r.tool.name).toSorted();
  ...
  it('registry discovers binary-instrument (which includes apk-packer)', async () => {
    const manifests = await discoverDomainManifests();
    const found = manifests.find((m) => m.domain === 'binary-instrument');
    expect(found).toBeDefined();
    expect(found?.depKey).toBe('binaryInstrumentHandlers');
  });

  it('end-to-end: detect via dirPath with customSignatures surfaces matches', async () => {
    const apkHandler = new ApkPackerHandlers();
    const dirPath = await makeApkDir({ 'arm64-v8a': ['libpacka_main.so', 'libpacka_helper.so'] });
    const response = await apkHandler.handleApkPackerDetect({
      dirPath,
      ruleMode: 'replace',
      customSignatures: [
        {
  ...
  it('end-to-end: list_signatures returns the empty default catalogue', async () => {
    const apkHandler = new ApkPackerHandlers();
    const response = await apkHandler.handleApkPackerListSignatures({});
